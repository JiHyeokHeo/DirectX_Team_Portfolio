...
// 라이브러리란?
// 자주 사용되는 기능이나 함수 서브루틴 등등등 가르켜서 총제적으로
// 구현해놓은 프로젝트이다.
...
// 실행한다.
// 하나의 메모리 영역에 올려두고 여러프로램들이 나누어 사용하는 
// 라이브러리

// 장점
// 메모리를 절약할 수 있다.
// 프로그램에 한번 올려진 메모리를 공유하여 사용하므로 사용공간자체가 적다.
// 업그레이드 (패치를) 비교적 정적 라이브러리보다 쉽게 진행 가능하다.
// 실행 중간에 패치가 가능하다.
...
// 정적 라이브러리
// 프로그램을 컴파일 하는 과정에 내가 작성한 라이브러리코드들이
// 포함된다.
// xxxx.lib
...
// lib 는 링크(linking)시 필요하고 dll 프로그램은 실행 시 필요하다.
